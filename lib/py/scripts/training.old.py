...
if len(sys.argv) > 1:
    ...
    plt.subplot(121)
    train_markersize = [20 for i in range(train_len)]
    plt.scatter(train_X, train_y, color="blue", s=train_markersize)
    ...
    # Plot Testing
    plt.subplot(122)
    test_markersize = [20 for i in range(test_len)]
    plt.scatter(test_X, test_y, color='blue', s=test_markersize)
    ...
    ##### needed sa paper na data
    # Print Data
    # print(f"{inputName} - {outputName}")
    # for xx, yy in zip(data_X, data_y):
    #     print(f"{xx[0]},{int(yy*100)}%")
    estimated_y = model.predict(data_X)
    print(title)
    for estimated, actual in zip(estimated_y, data_y):
        print(f"{int(estimated)},{int(actual)}")
